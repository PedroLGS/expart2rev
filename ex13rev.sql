...
SELECT c.marca, c.modelo, SUM(v.distanciaPercorrida) AS distancia_percorrida, e.nome AS nome_empresa
FROM carro c
INNER JOIN viagem v
ON c.id = v.idCarro
...
ORDER BY SUM(v.distanciaPercorrida) ASC

--Consultar Nomes das empresas que não tem carros cadastrados
SELECT e.nome 
FROM empresa e
LEFT OUTER JOIN carro c
ON e.id = c.idEmpresa
...
SELECT COUNT(v.distanciaPercorrida) AS num_viagens, c.marca, c.modelo, e.nome
FROM carro c
INNER JOIN empresa e
ON e.id = c.idEmpresa
INNER JOIN viagem v
ON v.idCarro = c.id
GROUP BY c.id, c.marca, c.modelo, e.nome
ORDER BY e.nome ASC
